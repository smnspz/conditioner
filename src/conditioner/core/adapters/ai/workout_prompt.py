...
def build_prompt(
    ...
    if "resistance_bands" in equipment_ids:
        equipment_notes += (
            "\nResistance bands are a full strength-training tool. Valid band exercises: "
            "rows, presses (chest/shoulder/incline), squats, deadlifts, Romanian deadlifts, "
            "pull-aparts, face pulls, curls, tricep extensions, lateral raises, hip abductions. "
            "Do NOT prescribe machine-style movements (leg press, leg extension, leg curl) with "
            "bands — they cannot be safely replicated. "
            "Do not default to cardio or mobility only because bands are the primary equipment. "
            "CRITICAL: any exercise that uses resistance bands MUST have equipment='resistance_bands'. "
            "Only use equipment='bodyweight' for exercises with zero equipment (e.g. push-ups, "
            "bodyweight squats, planks, burpees)."
        )
    ...
    return (
        ...
        f"{readiness_line}\n"
        "Only schedule sessions on weekdays with available minutes. "
        "Fill each session's time budget — a 60-minute session should total roughly "
        "60 minutes of exercise. Do not leave large portions of the time budget unused.\n"
        ...
        "Only set target_load when the equipment supports kg-based loading "
        "(barbells, dumbbells, kettlebells, weight machines). "
        "For resistance bands and pure bodyweight exercises, always set target_load to null.\n"
        "Modality rules: use 'strength' for resistance exercises including planks, "
        "push-ups, and core holds. Use 'cardio' for aerobic/metabolic work (burpees, "
        "jumping jacks, mountain climbers, sprints). Use 'mobility' only for stretching "
        "and joint preparation. "
        "Every strength exercise MUST include both sets and reps — no exceptions. "
        "For isometric holds (e.g. plank), express as sets × reps where reps = seconds "
        "(e.g. 3 sets × 30 reps = 3 × 30-second holds).\n"
