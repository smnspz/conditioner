...
Weekly workout plans are generated by prompting the Gemini API (see `core/adapters/ai/gemini`). Only a user's workout constraints (equipment, goal, available time) and their readiness score/zone are sent — raw wearable metrics and questionnaire answers never leave the system. This still constitutes personal data leaving the EU-hosted system to a third-party processor; lawful basis and Data Processing Agreement coverage for this processor must be confirmed before this adapter is wired into any live use case (task 7b).
