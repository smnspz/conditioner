...
from conditioner.core.domain.workout.workout import ExerciseModality, Workout
...
# 5 minimal catalog entries so the minimum-catalog-size guard passes
_CATALOG = [
...
def _repos(*, constraints=_CONSTRAINTS, fitness_level=_FITNESS_LEVEL, readiness=_READINESS,
    ...
    catalog_repository = AsyncMock(filter_by_gear=AsyncMock(return_value=catalog))
    return (
...
async def test_raises_when_too_few_catalog_entries() -> None:
    constraints_repo, fitness_repo, readiness_repo, provider, workout_repo, catalog_repo = _repos(
        catalog=_CATALOG[:3]
    )

    with pytest.raises(PrerequisitesMissingError, match="Not enough exercises"):
        await generate_weekly_plan(
            _USER_ID, _WEEK_START, constraints_repo, fitness_repo, readiness_repo, provider,
            workout_repo, catalog_repo,
        )
